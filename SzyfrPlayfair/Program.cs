...
class Playfair
{
	...
	private static char[] SameRow(ref char[,] keySquare, int row, int col1, int col2, int encipher)
		=> new char[] { keySquare[row, Mod((col1 + encipher), 5)], keySquare[row, Mod((col2 + encipher), 5)] };

	private static char[] SameColumn(ref char[,] keySquare, int col, int row1, int row2, int encipher)
		=> new char[] { keySquare[Mod((row1 + encipher), 5), col], keySquare[Mod((row2 + encipher), 5), col] };

	private static char[] SameRowColumn(ref char[,] keySquare, int row, int col, int encipher)
		=> new char[] { keySquare[Mod((row + encipher), 5), Mod((col + encipher), 5)], keySquare[Mod((row + encipher), 5), Mod((col + encipher), 5)] };

	private static char[] DifferentRowColumn(ref char[,] keySquare, int row1, int col1, int row2, int col2)
		=> new char[] { keySquare[row1, col2], keySquare[row2, col1] };
...
class Program
{
	public static void Main()
    {
		...
		string encrypted = Playfair.Encrypt(input, key);
		string decrypted = Playfair.Decrypt(encrypted, key);
		Console.WriteLine($" Input: {input}");
		Console.WriteLine($" Cipher: {encrypted}");
		Console.WriteLine($" Decipher: {decrypted}");
	}
}
